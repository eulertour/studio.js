...
const ignoreRegexes = [/.*node_modules.*/, /.*examples\/dist.*/, /.*examples\/build.*/];
const currentDirectory = dirname(fileURLToPath(import.meta.url));
const app = initializeApp(config);
const bucket = getStorage(app).bucket();

await uploadDirectory(join(currentDirectory, '../../build'), bucket, ignoreRegexes);

await uploadDirectory(join(currentDirectory, '../../src'), bucket, ignoreRegexes);

await uploadDirectory(join(currentDirectory, '../../examples'), bucket, ignoreRegexes);

console.log('Done');

type Bucket = typeof bucket;

async function uploadDirectory(directory: string, bucket: Bucket, ignoreRegexes: RegExp[]) {
