...
async function* getFiles(directory = ".", ignoreRegexes) {
...
async function uploadDirectory(directory, bucket, ignoreRegexes) {
    for await (const filePath of getFiles(directory, ignoreRegexes)) {
        try {
            const source = filePath;
            const destination = path.join("studio.js", path.basename(directory), path.relative(directory, filePath));
