...
export function loadSVG(options) {
    const { url, scene, scale = new THREE.Vector3(0.01, -0.01, 0.01), position = new THREE.Vector3(0, 0, 0), center = true, onProgress } = options;
...
export function loadText(options) {
    const { url, scene, text, size = 0.5, depth = 0.1, position = new THREE.Vector3(0, -2, 0), scale = new THREE.Vector3(3, 3, 3), color = "white", onProgress } = options;
    return new Promise((resolve, reject) => {
