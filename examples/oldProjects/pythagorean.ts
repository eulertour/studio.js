...
export default class Example implements StudioScene {
	...
	animateSquare() {
		...
		this.animations.push(
			{
				animations: [new Animation.Draw(triangle), new Animation.FadeIn(this.triangleLabels[0])],
			},
			new Animation.Shift(this.triangles[0], triangleCenter),
			{
				...
				animations: [
					...this.triangles
						.slice(1)
						.map((triangle, i) => new Animation.Rotate(triangle, (-(i + 1) * Math.PI) / 2)),
	...
	animateArea() {
		...
		Utils.moveToLeftOf(this.indicator, indicatorLabel, this.indicatorLabelOffset);
		...
		this.animations.push(
			...
			[
				...
				new Animation.Shift(this.diagram, new THREE.Vector3().addScaledVector(Utils.LEFT, 3.3)),
			],
		);

		this.equations = new THREE.Group().translateX(3.3).translateY(1.9).translateY(-1.9);
		this.equations.setScale(0.5);

		const line1 = new Text.Text('(a + b)^2 = c^2 + 4\\left(\\frac{1}{2}ab\\right)', {
			groupColoring: [
				[6, 'red'],
				[7, 'black'],
				[9, 'green'],
		...
		const line2 = new Text.Text('a^2 + 2ab + b^2 = c^2 + 4\\left(\\frac{1}{2}ab\\right)');
	...
	getLabels(triangle: Geometry.Polygon): THREE.Group {
		...
		for (let i = 0; i < 3; i++) {
			sideVectors.push(new THREE.Vector3().subVectors(triangle.points[i + 1], triangle.points[i]));
		}
		...
		labelGroup.children.forEach((letter, i) =>
			letter.position.copy(midpoints[i]).addScaledVector(normals[i], this.labelOffset),
		);
