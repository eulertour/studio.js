...
const transformBetweenSpaces = (from: THREE.Object3D, to: THREE.Object3D, point: THREE.Vector3) => {
...
const furthestInDirection = (
	...
	object.traverse((obj) => {
		...
		} else if (obj instanceof THREE.Mesh && obj.parent?.parent?.parent instanceof Text.Text) {
...
const shapeIsClosed = (shape, adjacentThreshold = 0.0001) => {
	return (
		new THREE.Vector3().subVectors(shape.points.at(0), shape.points.at(-1)).length() <
		adjacentThreshold
	);
};
...
class ShapeFromCurves {
	...
	extendAlong(shape: Geometry.Shape, direction: THREE.Vector3, until?: THREE.Vector3 | undefined) {
	...
	extendCurve(
		...
		while (0 <= i && i < shape.points.length) {
			...
			const newPoint = shape.points.at(i)?.clone().applyMatrix4(shape.matrixWorld);
