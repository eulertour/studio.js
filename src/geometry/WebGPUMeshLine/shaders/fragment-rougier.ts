...
import { PatternAtlas, patternAtlas } from "./atlas-rougier.js";
  ...
  buildFragmentNode() {
    return Fn(
      () => {
        ...
        const v = texture(
          patternAtlas.atlas,
          vec2(u2.div(freqPatternLength), 0),
        );
        ...
        If(segmentStart.notEqual(lineStart), () => {
          ...
          If(dashStart.lessThanEqual(segmentStart), () => {
            const previousFragment = varyingProperty(
              "vec2",
              "vPreviousFragment",
            );
            ...
            const fragmentHalfWidth = halfWidth.mul(
              segmentFragmentsPerDistance,
            );
        ...
        // return color;
        return vec4(this.strokeColor, this.strokeOpacity);
      },
    );
  }
}
