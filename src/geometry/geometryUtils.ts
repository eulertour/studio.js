...
export const getEllipseArcPoints = (radiusA: number, radiusB: number, angle: number, config: { closed?: boolean } = {}) => {
  let points = [];
  let negative = false;
  if (angle < 0) {
    negative = true;
  ...
  if (angle > 0) {
    for (let i = 0; i <= angle + ERROR_THRESHOLD; i += angle / 50) {
      points.push(
        new THREE.Vector3(
          radiusB * Math.cos(i),
          radiusA * Math.sin(i) * (negative ? -1 : 1),
          0,
        ),
      );
    }
    ...
    points.push(
      new THREE.Vector3(radiusB, radiusA, 0),
      new THREE.Vector3(radiusB, radiusA, 0),
    );
  }
  if (config.closed) {
