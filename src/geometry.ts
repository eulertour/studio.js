...
import { MeshLine, MeshLineMaterial, MeshLineRaycast } from './THREE.MeshLine.dev.js';
import type {
	Transform,
	Style,
	StyleJson,
...
abstract class Shape extends THREE.Group {
	...
	static jsonToStyle = (styleJson: StyleJson): Style => {
		const { strokeColor, strokeOpacity, strokeWidth, stroke, fillColor, fillOpacity, fill } =
			styleJson;
		return {
...
class Line extends Shape {
	constructor(
		...
		if (config.transformCenter) {
			...
			super(newPoints, { ...config, fill: false });
			this.position.copy(strokeCenter);
		} else {
			super([start, end], { ...config, fill: false });
		}
...
class Point extends Circle {
	...
	constructor(public location: THREE.Vector2 | THREE.Vector3, config: Style = {}) {
		super(0.08, {
